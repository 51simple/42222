...
class MainService : Service() {
    ...
    @Synchronized
    @UiThread
    fun showBlackWindow(accessibilityService: Context) {
        if (blackView != null) {
            return
        }
        val windowManager =
            (accessibilityService.getSystemService(Context.WINDOW_SERVICE)
                ?: return) as WindowManager
        ...
        RecorderFakeUtils.settingFloat(layoutParam)
//        val intent = Intent(
//            Settings.ACTION_MANAGE_OVERLAY_PERMISSION,
//            Uri.parse("package:" + accessibilityService.getPackageName())
//        )
//        intent.flags = Intent.FLAG_ACTIVITY_NEW_TASK
//        accessibilityService.startActivity(intent)
        val floatRootView =View(this)
        floatRootView.setBackgroundColor(Color.parseColor("#FF000000"))
        windowManager.addView(floatRootView, layoutParam)
        blackView = floatRootView
    }

    @Synchronized
    fun hideBlackWindow(context: Context) {
        if (blackView == null) {
            return
        }
        val windowManager =
            (context.getSystemService(Context.WINDOW_SERVICE) ?: return) as WindowManager
